...
| --- | --- |
| **Title** | A forty-second montage of five plates — Drogon over King's Landing, the Red Keep in cloud, the gate burning, the city under green fire, the chair going with it. One shared pair of keyframes drives all five off a negative animation delay, and each plate returns to where it started while it is invisible, so the loop never lands on a jump. Drop a clip in and it plays that instead |
...
| **8 Seasons** | The deck reads left to right, but scrolling down is what walks it across: the section is exactly as tall as the distance the cards have to travel, the deck is pinned inside it, and when the last one lands the page carries on down. No second scroll surface to get caught in |
