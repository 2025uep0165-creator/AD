...
/* ---------- palette ---------- */
const BRICKS = ['#a85c3c','#b96b47','#9c5134','#c47c56','#8f4a2e','#ae6340'];
const CONCRETE = ['#9aa0a6','#8b9197','#a6adb3'];
...
export function createScene(canvas, opts = {}) {
  ...
  scene.fog = new THREE.Fog(0x131a20, 120, 280);

  const FRAME = lowPower ? 1.34 : 1;                       // pull back on small screens
  const camera = new THREE.PerspectiveCamera(lowPower ? 46 : 38, 1, 0.5, 400);

  /* ---------- light ---------- */
  const hemi = new THREE.HemisphereLight(0xb8d4ea, 0x4a5560, 1.05);
  scene.add(hemi);

  const sun = new THREE.DirectionalLight(0xffe6c4, 2.9);
...
function loadProps(group, props, onWater, lowPower) {
  ...
  }, undefined, err => console.warn('[ouroboros-arq] props unavailable; the brick build stands alone.', err));
}
...
function skyTexture() {
  ...
  g.addColorStop(0.00, '#101c2c');
  g.addColorStop(0.34, '#28374a');
  g.addColorStop(0.52, '#4a5a6b');
  g.addColorStop(0.62, '#5d7186');
  g.addColorStop(0.74, '#28323d');
  g.addColorStop(1.00, '#0a1119');
...
function groundTexture() {
  ...
  x.fillStyle = '#1b1f26'; x.fillRect(0, 0, S, S);
  for (let i = 0; i < 26000; i++) {                       // grit
    x.fillStyle = `rgba(${118 + Math.random() * 60 | 0},${126 + Math.random() * 56 | 0},${140 + Math.random() * 50 | 0},${Math.random() * 0.22})`;
    x.fillRect(Math.random() * S, Math.random() * S, 2, 2);
  }
  x.strokeStyle = 'rgba(150,190,230,0.20)'; x.lineWidth = 1;   // survey grid
  for (let i = 0; i <= 32; i++) {
    const p = (i / 32) * S;
