/* mini-construction — application logic. */

import { WORKS } from './works.js';
import { BRAND, FIGURES, CHAPTERS, DISCIPLINES, TECHNIQUES, TIMELINE, PLATFORMS, FILTERS, MOTION, STACK } from './site.js';
...
/* category label lookup, derived from the filter list so the two can never drift */
const CATNAME = Object.fromEntries(
  FILTERS.filter(f => f.k !== 'all').map(f => [f.k, f.label]));
...
/* -- marquee of real thumbnails --
   Channels that publish long-form films have few (or no) vertical posters, so the
   poster bands take Shorts first and top up with films rather than running empty. */
const SHORTS = WORKS.filter(w => w.f === 'short');
const WIDE = SHORTS.length < WORKS.length / 2;          // film-first catalogue
const posters = n => [...SHORTS, ...WORKS.filter(w => w.f !== 'short')].slice(0, n);
const strip = posters(30);
if (WIDE) { $('.marquee').classList.add('is-wide'); $('#wallGrid').classList.add('is-wide'); }
$('#marqueeTrack').innerHTML = [...strip, ...strip]
  .map(w => `<img src="${thumb(w)}" alt="" loading="lazy" decoding="async" ${onerr(w)}>`).join('');

/* -- craft stack -- */
const stackPicks = STACK.map(id => WORKS.find(w => w.id === id)).filter(Boolean);
...
$('#discList').innerHTML = discData.map((d, i) => `
  <li class="di rv${i === 0 ? ' is-on' : ''}" data-cat="${d.k}" data-i="${i}" tabindex="0" role="button"
    ...
    <span class="di__c mono"><b>${d.n}</b> projects<i></i><b>${big(d.views)}</b> views</span>
  </li>`).join('');
...
/* -- "In motion": three projects that play on loop where they stand -- */
...
function renderGallery() {
  ...
  workCount.textContent = `Showing ${list.length} of ${view.length} projects`;
}
applyFilter('all');
...
async function startScene() {
  ...
  try {
    ...
    console.warn('[mini-construction] 3D stage unavailable — falling back to the still stage.', err);
...
console.log(`%c${BRAND.name}%c — ${BRAND.tagline}\nArchive: ${WORKS.length} projects · since ${BRAND.joined}`,
