...
/* ---------- palette ---------- */
const BRICKS = ['#8d7f6c','#9c8e79','#7d7061','#ab9d86','#736759','#948673'];
const CONCRETE = ['#8e948d','#7f857e','#99a098'];
...
function loadProps(group, props, onWater, lowPower) {
  ...
  }, undefined, err => console.warn('[mini-construction] props unavailable; the brick build stands alone.', err));
}
...
function skyTexture() {
  ...
  g.addColorStop(0.00, '#10202a');
  g.addColorStop(0.34, '#26424a');
  g.addColorStop(0.52, '#3f6157');
  g.addColorStop(0.62, '#557a63');
  g.addColorStop(0.74, '#23382f');
  g.addColorStop(1.00, '#0a1512');
...
function groundTexture() {
  ...
  x.fillStyle = '#16211c'; x.fillRect(0, 0, S, S);
  for (let i = 0; i < 26000; i++) {                       // grit
    x.fillStyle = `rgba(${104 + Math.random() * 56 | 0},${126 + Math.random() * 56 | 0},${112 + Math.random() * 48 | 0},${Math.random() * 0.22})`;
    x.fillRect(Math.random() * S, Math.random() * S, 2, 2);
  }
  x.strokeStyle = 'rgba(140,220,195,0.20)'; x.lineWidth = 1;   // survey grid
  for (let i = 0; i <= 32; i++) {
    const p = (i / 32) * S;
