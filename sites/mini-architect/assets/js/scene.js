/* mini-architect — "The Build".
   A miniature building that assembles itself, course by course, as you scroll.
...
/* ---------- palette ---------- */
const BRICKS = ['#b05a33','#c06a41','#a34f2c','#cb7e54','#96482a','#b5623a'];
const CONCRETE = ['#8a8a86','#7c7c78','#93938f'];
...
export function createScene(canvas, opts = {}) {
  ...
  scene.fog = new THREE.Fog(0x14120f, 120, 280);

  const FRAME = lowPower ? 1.34 : 1;                       // pull back on small screens
  const camera = new THREE.PerspectiveCamera(lowPower ? 46 : 38, 1, 0.5, 400);

  /* ---------- light ---------- */
  const hemi = new THREE.HemisphereLight(0xbcc8d4, 0x4e463c, 1.05);
  scene.add(hemi);

  const sun = new THREE.DirectionalLight(0xffdfb2, 2.9);
...
function loadProps(group, props, onWater, lowPower) {
  ...
  new GLTFLoader().load('assets/models/props.glb', gltf => {
  ...
  }, undefined, err => console.warn('[mini-architect] props unavailable; the brick build stands alone.', err));
}
...
function skyTexture() {
  ...
  g.addColorStop(0.00, '#141620');
  g.addColorStop(0.34, '#32333c');
  g.addColorStop(0.52, '#5d5244');
  g.addColorStop(0.62, '#7d6842');
  g.addColorStop(0.74, '#332c22');
  g.addColorStop(1.00, '#100e0b');
...
function groundTexture() {
  ...
  x.fillStyle = '#1c1a16'; x.fillRect(0, 0, S, S);
  for (let i = 0; i < 26000; i++) {                       // grit
    x.fillStyle = `rgba(${126 + Math.random() * 68 | 0},${112 + Math.random() * 56 | 0},${86 + Math.random() * 46 | 0},${Math.random() * 0.22})`;
    x.fillRect(Math.random() * S, Math.random() * S, 2, 2);
  }
  x.strokeStyle = 'rgba(235,192,103,0.20)'; x.lineWidth = 1;   // survey grid
  for (let i = 0; i <= 32; i++) {
    const p = (i / 32) * S;
