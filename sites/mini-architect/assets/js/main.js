/* mini-architect — application logic. */
...
$('#discList').innerHTML = discData.map((d, i) => `
  <li class="di rv${i === 0 ? ' is-on' : ''}" data-cat="${d.k}" data-i="${i}" tabindex="0" role="button"
    ...
    <span class="di__c mono"><b>${d.n}</b> films<i></i><b>${big(d.views)}</b> views</span>
  </li>`).join('');
...
/* -- "In motion": three films that play on loop where they stand -- */
...
function renderGallery() {
  ...
  workCount.textContent = `Showing ${list.length} of ${view.length} films`;
}
applyFilter('all');
...
async function startScene() {
  ...
  try {
    ...
    console.warn('[mini-architect] 3D stage unavailable — falling back to the still stage.', err);
...
console.log(`%c${BRAND.name}%c — ${BRAND.tagline}\nArchive: ${WORKS.length} films · since ${BRAND.joined}`,
