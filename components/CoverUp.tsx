 ...
 * it is also a real ARIA slider so it works from the keyboard.
 *
 * What it compares is coverage, not one arm before and after — see the note on
 * `coverUp` in lib/content.ts for why, and what to change when a real pair
 * arrives.
 */
      ...
      <div className="u-gutter grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-center lg:gap-16">
        ...
        <figure className="m-0">
          <div
            ...
            role="slider"
            tabIndex={0}
            aria-label={`Compare ${coverUp.leftLabel} against ${coverUp.rightLabel}`}
            aria-valuemin={0}
            aria-valuemax={100}
          ...
          >
            ...
            <div
              ...
              aria-hidden="true"
            >
              <Frame media={coverUp.left} sizes="(min-width: 1024px) 46vw, 100vw" />
            </div>
            ...
            <span className="u-mono absolute left-3 top-3 bg-bone/90 px-2 py-1">
              {coverUp.leftLabel}
            </span>
            <span className="u-mono absolute right-3 top-3 bg-ink/90 px-2 py-1 text-bone">
              {coverUp.rightLabel}
            </span>
          </div>
